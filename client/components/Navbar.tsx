...
import { Menu, SunIcon, User } from "lucide-react";
...
const Navbar = () => {
  ...
  return (
    <header className=" flex justify-between items-center py-3 px-[3%] fixed top-0 left-0 w-full z-50 bg-transparent/30 backdrop-blur-xs border-b border-white/10 shadow-xs mt-2">
      {/* logo */}
      ...
      {/* menus */}
      <nav className=" hidden md:flex justify-between items-center max-w-xl w-full py-2 px-8 bg-white/75 shadow-sm rounded-4xl">
        {menu.map((item) => {
                    ...
                    <Link
                      ...
                      className={` font-medium text-sm p-2.5 rounded-md ${
                        isActive
                          ...
                          : "text-gray-800 transition-colors ease-in"
                      } `}
                    >
                      {item.name}
                    </Link>
